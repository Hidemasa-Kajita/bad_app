...
- 初期ユーザー確認方法
- 診断用アプリのセットアップ方法
  - firebox@http://web:3030 で画面の表示が確認できるまで
- 脆弱性診断ツールのセットアップ方法
...
### セットアップ
- firebox install+プロキシの設定
https://qiita.com/sangi/items/ba7e3d39237045c9be36  
// 画像を付ける
- 以下コマンドでappのbuild
  - dockerコンテナ作成~アプリの起動までやってくれます
```
make setup
```

- それぞれ接続が遅いので何十回かリロードしたりしてください。 
  - chome@http://localhost:8081/zap/
    - OWASPの画面が出た後リロードすると落ちるので注意してください。その場合、`make down && make up`を行ってください。
  - firebox@http://web:3030

### 初期ユーザーの確認方法
- ユーザー登録もできますが、以下のエンドポイントから初期ユーザーを確認できます。
firebox@http://web:3030/api/users
